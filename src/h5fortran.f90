module h5fortran
    ...
    use hdf5, only : HID_T, SIZE_T, HSIZE_T, H5F_ACC_RDONLY_F, H5F_ACC_RDWR_F, H5F_ACC_TRUNC_F, &
                     H5S_ALL_F, H5S_SELECT_SET_F, H5D_CONTIGUOUS_F, H5D_CHUNKED_F, H5D_COMPACT_F, &
                     H5T_NATIVE_DOUBLE, H5T_NATIVE_REAL, H5T_NATIVE_INTEGER, H5T_NATIVE_CHARACTER, &
                     H5F_SCOPE_GLOBAL_F, h5open_f, h5close_f, h5dopen_f, h5dclose_f, h5dget_space_f, &
                     h5gcreate_f, h5gclose_f, h5fopen_f, h5fcreate_f, h5fclose_f, h5fis_hdf5_f, &
                     h5lexists_f, h5sclose_f, h5sselect_hyperslab_f, h5screate_simple_f, &
    ...
    type :: hdf5_file
        ...
        integer(HID_T) :: glid    !< group location ID
    ...
    contains
        ...
        procedure, public :: finalize   => hdf_finalize, write_group 
        procedure, public :: create => hdf_create
        procedure, public :: open => hdf_open_group 
        procedure, public :: close => hdf_close_group 
        ...
        !> read dataset
        generic, public :: read => hdf_read_scalar, hdf_read_1d, hdf_read_2d, hdf_read_3d, hdf_read_4d, &
                                   hdf_read_5d, hdf_read_6d, hdf_read_7d

        !> private methods
        !! each method must be declared here, and above as a generic, public
        procedure,private ::  hdf_write_scalar, hdf_write_1d, hdf_write_2d, hdf_write_3d, hdf_write_4d, & 
                              hdf_write_5d, hdf_write_6d, hdf_write_7d, hdf_read_scalar, hdf_read_1d, &
                              hdf_read_2d, hdf_read_3d, hdf_read_4d, hdf_read_5d, hdf_read_6d, hdf_read_7d, &
    ...
    interface !< write.f90
        ...
        module subroutine create_softlink(self, target, link)
            ...
            character(*), intent(in) :: link    !< soft link path to create
...
contains
    ...
    subroutine hdf_shape_check(self, dname, dims)
        ...
        select type (dims)
            ...
            class default
                write(stderr,*) & 
                    'ERROR:h5fortran:shape_check: wrong type for dims: ', dname, self%filename
                error stop 5
        end select
        ...
        if (ierr/=0) then
            write(stderr,*) & 
                'ERROR:h5fortran:shape_check: get_dataset_ndim '//dname//' read '//self%filename
            error stop
        endif
        
        if (drank /= size(vdims)) then
            write(stderr, '(A,I6,A,I6)') & 
                'ERROR:h5fortran:shape_check: rank mismatch '//dname//' = ',drank,'  variable rank =',size(vdims)
            error stop
        endif
        ...
        if (ierr/=0) then
            write(stderr,*) & 
                'ERROR:h5fortran:shape_check: get_dataset_info '//dname//' read '//self%filename
            error stop
        endif
        
        if(.not. all(vdims == ddims)) then
            write(stderr,*) & 
                'ERROR:h5fortran:shape_check: shape mismatch '//dname//' = ',ddims,'  variable shape =',vdims
            error stop
        endif
