...
contains
    ...
    module procedure hdf_read_scalar
        ...
        if(.not.self%is_open) then 
            error stop 'h5fortran:reader: file handle is not open'
        end if
        
        space_id = 0
        
        if (.not.self%exist(dname)) then 
            error stop 'h5fortran:reader: ' // dname // ' does not exist in ' // self%filename
        end if
        
        call h5dopen_f(self%lid, dname, ds_id, ier)
        if(ier/=0) then 
            error stop 'h5fortran:reader: ' // dname // ' could not be opened in ' // self%filename
        end if 
        ...
        select type (value)
            type is (character(*)) 
                ...
                if(native_dtype /= H5T_NATIVE_CHARACTER) then 
                    error stop "h5fortran:reader: non-character variable: " // dname//" in "//self%filename
                end if
                block
        ...
        if(ier/=0) then 
            error stop 'h5fortran:reader: reading ' // dname // ' from ' // self%filename
        end if
    ...
    module procedure hdf_read_7d
        ...
        if(native_dtype == H5T_NATIVE_DOUBLE) then
            ...
            select type(value)
                ...
                type is (real(real32))
                    allocate(buf_r64(size(value, 1), size(value, 2), size(value, 3), size(value, 4), & 
                        size(value, 5),size(value, 6),size(value, 7)))
                    call h5dread_f(ds_id, H5T_NATIVE_DOUBLE, buf_r64, dims, ier, mem_space_id, space_id)
                    value = real(buf_r64, real32)
                type is (integer(int32))
                    allocate(buf_r64(size(value, 1), size(value, 2), size(value, 3), size(value, 4), & 
