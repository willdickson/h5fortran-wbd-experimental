submodule (h5fortran) read
    ...
    use hdf5, only : h5dget_create_plist_f, h5pget_layout_f, h5pget_chunk_f, &
                     h5dget_type_f, h5tget_native_type_f, h5tget_class_f,  & 
                     H5Tget_order_f, h5tclose_f, h5tget_size_f, H5T_DIR_ASCEND_F, & 
                     H5T_INTEGER_F, H5T_FLOAT_F, H5T_STRING_F, H5T_STD_I64LE
...
contains
    ...
    integer(hid_t) function get_native_dtype(ds_id, dname, filename) result(native_dtype)
        ...
        if(ierr/=0) then 
            error stop 'h5fortran:reader: get internal dtype ' // dname // ' from ' // filename
        end if

        call h5tget_native_type_f(dtype_id, H5T_DIR_ASCEND_F, native_dtype_id, ierr)
        if(ierr/=0) then 
            error stop 'h5fortran:reader: get native dtype id ' // dname // ' from ' // filename
        end if
        ...
        ! if(order /= machine_order) error stop 'h5fortran:reader: endianness does not match machine 
        ! native ' // dname // ' from ' // filename

        !> compose datatype inferred
        call h5tget_class_f(native_dtype_id, class, ierr)
        if(ierr/=0) then 
            error stop 'h5fortran:reader: get class ' // dname // ' from ' // filename
        end if

        call h5tget_size_f(native_dtype_id, size_bytes, ierr)
        if(ierr/=0) then 
            error stop 'h5fortran:reader: get byte size ' // dname // ' from ' // filename
        end if

        call h5tclose_f(dtype_id, ierr)
        if(ierr/=0) then 
            error stop 'h5fortran:reader: closing dtype ' // dname // ' from ' // filename
        end if
